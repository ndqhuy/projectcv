        ...
        <table class="table table-striped rounded mt-3">
            ...
            <tbody>
                ...
                @foreach ($userSavedJobs as $userSavedJob)
                    <tr>
                        <td>{{ $index++ }}</td>
                        <td>{{ $userSavedJob->users->username }}</td>
                        <td>
                            <a href="{{ route('jobDetail.dashboard', ['id' => $userSavedJob->job->job_id]) }}" class="text-dark"
                                style="text-decoration: none">{{ $userSavedJob->job->job_name }}</a>
                        </td>
                        <td>
                            {{-- <a href="">Update</a> --}}
